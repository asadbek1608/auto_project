...
const forgotPassword = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ message: "change pasword code sent" });
  } catch (error) {
    next(error);
  }
};
...
const forgotPasswordChange = async (req, res, next) => {
  try {
    ...
    if (newPassword !== confirmPassword) {
      throw BaseError.BadRequest("Passwords do not match");
    }
    ...
    res.status(200).json({ message: "password sucsefully changed" });
  } catch (error) {
    next(error);
  }
};
